...
//          $myBetModel = DB::select("UPDATE `my_bets` SET `active` = '1' WHERE `id` IN(".$betIDs.")");
    ...
    public function store(Request $request){
      ...
      if(isset($gameModel) && is_object($gameModel)){
        ...
        $extra1 = json_encode($arr['bteamname']);
      }
      $model = new Sports();
...
//      $model->tv_url = $request->input('tv_url');
    ...
    public function winnerselect(Request $request){
      ...
      switch ($gameName){
        case 'CRICKET':{
//          $url = 'http://172.105.37.187/json/1.'.$sports->match_id.'.json';
//          $ch = curl_init( $url );
...
//          $selectOptin .= '<option value="Draw">Draw</option>';
        ...
        case 'TENNIS':{
//          $url = 'http://172.105.57.35/json/1.'.$sports->match_id.'.json';
//          $ch = curl_init( $url );
//          curl_setopt( $ch, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));
//          curl_setopt( $ch, CURLOPT_RETURNTRANSFER, true );
//          $result = curl_exec($ch);
...
//          $selectOptin .= '<option value="Draw">Draw</option>';
        ...
        case 'SOCCER':{
//          $url = 'http://45.79.123.242/json/1.'.$sports->match_id.'.json';
//          $ch = curl_init( $url );
//          curl_setopt( $ch, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));
//          curl_setopt( $ch, CURLOPT_RETURNTRANSFER, true );
//          $result = curl_exec($ch);
    ...
    public static function getTeamName($gameName,$MatchID){
      ...
      switch ($gameName){
        case 'CRICKET':{
          ...
          $dataArr['bteamname'][1] = isset($arr['bookmake'][0]['runners'][0]['RunnerName']) ? $arr['bookmake'][0]['runners'][0]['RunnerName'] : '';
          $dataArr['bteamname'][2] = isset($arr['bookmake'][0]['runners'][1]['RunnerName']) ? $arr['bookmake'][0]['runners'][1]['RunnerName'] : '';
          $team3 = isset($arr['bookmake'][0]['runners'][2]['RunnerName']) ? $arr['bookmake'][0]['runners'][2]['RunnerName'] : '';
          if(!empty($team3)){
            $dataArr['bteamname'][3] = $team3;
          }
          break;
        }
        case 'TENNIS':{
           $url = 'http://172.105.57.35/json/1.'.$MatchID.'.json';
          $ch = curl_init( $url );
          curl_setopt( $ch, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));
          curl_setopt( $ch, CURLOPT_RETURNTRANSFER, true );
          $result = curl_exec($ch);
        ...
        case 'SOCCER':{
          $url = 'http://45.79.123.242/json/1.'.$MatchID.'.json';
          $ch = curl_init( $url );
          curl_setopt( $ch, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));
          curl_setopt( $ch, CURLOPT_RETURNTRANSFER, true );
          $result = curl_exec($ch);
...
//      $sportss = DB::select( "SELECT * FROM `sports` WHERE match_id='".$token."' AND active='1' AND (winner ='' OR winner IS NULL) LIMIT 1");
      ...
      $url = 'http://172.105.37.187/json/1.'.$token.'.json';
          $ch = curl_init( $url );
          curl_setopt( $ch, CURLOPT_HTTPHEADER, array('Content-Type:application/json'));
          curl_setopt( $ch, CURLOPT_RETURNTRANSFER, true );
          $result = curl_exec($ch);
...
//        $mybetsModel = MyBets::where(['match_id'=>$matchID,'active'=>'1','isDeleted'=>0])->orderBy('id', 'DESC')->get();
    ...
    public function getcasinoData(Request $request){
      ...
      switch ($requestData['gameName']){
        ...
        case 'AndarBahar':{
          $url = 'http://172.104.206.64/json/andar_bahar.json';
          break;
        }
        case 'Poker':{
          $url = 'http://172.104.206.64/json/poker.json';
          break;
        }
        case 'UpDown7':{
        ...
        case 'AmarAkbarAnthony':{
          $url = 'http://172.104.206.64/json/amarakbar.json';
          break;
        }
        case 'DragOnTiger':{
          $url = 'http://172.104.206.64/json/dragont.json';
          break;
        }
      }
