    ...
    var player;
        var config={"source":{"h5live": {
            "rtmp": {
                ...
                "streamname": "zWo7p-dASHi","type":"live"
            },
            "server": {
...
<script>
  ...
  function getData(){
    $.ajax({
      ...
      cache: false,
      async: false, 
      data : "gameName=UpDown7&sportID={{$sports->id}}&_token={{csrf_token()}}",
      success: function(data){
        setDetails(data.detail,'UpDown7');
        setodd(data.runner,'UpDown7');
        setResult(data.result,'UpDown7');
        getBetsList();
      }
    });
  }
