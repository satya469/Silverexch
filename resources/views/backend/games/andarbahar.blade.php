              ...
              <div data-title="OPEN" class="table-body  ODDS">
                <div data-title="ACTIVE" class="table-row oddsTot oddssteam0">
                  <div class="float-left country-name box-4">
                    ...
                    <p>
                        <span class="float-left matchValClear" style="color: black;"></span> 
                        <span class="float-right matchValClear" style="display: none; color: black;"></span>
                    </p>
                  </div>
   ...
   var player;
        var config={"source":{"h5live": {
            "rtmp": {
                ...
                "streamname": "zWo7p-RU8fy","type":"live"
            },
            "server": {
...
<script>
  ...
  function getData(){
    $.ajax({
      ...
      cache: false,
      async: false, 
      data : "gameName=AndarBahar&sportID={{$sports->id}}&_token={{csrf_token()}}",
      success: function(data){
        setDetails(data.detail,'AndarBahar');
        setodd(data.runner,'AndarBahar');
        setResult(data.result,'AndarBahar');
        getBetsList();
      }
    });
  }
