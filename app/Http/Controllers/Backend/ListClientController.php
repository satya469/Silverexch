...
class ListClientController extends Controller
{
    ...
    public static function getNumberFormet($num = 0){
        if($num == 0 || empty($num)){
            return 0;
        }
    ...
    public function listclients()
    {
        ...
        if(Auth::user()->roles->first()->name == 'administrator'){
            ...
            if(isset($userPrivilegeCheckModel->parent_id) && $userPrivilegeCheckModel->parent_id > 0){
            $parentID = $userId = $userPrivilegeCheckModel->parent_id;
            Session::put('parent_id', $userPrivilegeCheckModel->parent_id);
            }else{
        ...
        $clients = User::where(['parent_id'=>$userId])->orderBy('betActive', 'DESC')->get();
        $mainCalArr = array();
        $isParent = true;
    ...
    public static function getDepositeBalnceADMIN($userID){
...
//        dd($pl);
    ...
    public static function getPLBalanceADMIN($userID){
        $model = User::find($userID);
        $dep = self::getDepositeBalnceADMIN($userID);
        $plAmt = $dep-$model->credit_ref;
        return $plAmt;
    }
    public static function getUserAvalableBalanceADMIN($userID){
        $model = User::find($userID);
        $depParent = ListClientController::getDepositeBalnceADMIN($userID);
        $pLTot = ListClientController::getPLBalanceADMIN($userID);
        $exposureTot = ListClientController::getExAmountPerByUser($userID);
        $abc = self::getChildAvalableBalanceADMIN($userID);
        if($pLTot > 0){
            return ($depParent-($pLTot+$exposureTot+$abc));
        }else{
            return (($model->credit_ref-abs($pLTot))-($exposureTot+$abc));
        }
        
        
        
    }
    public static function OLD_getUserAvalableBalanceADMIN($userID){
        
        
        $client = User::find($userID);
        $depParent = DB::table('user_deposites')->where(['deposite_user_id'=>$client->id,'withdrawal_user_id'=>$client->parent_id])->sum('amount');
        $widTot = DB::table('user_deposites')->where(['withdrawal_user_id'=>$client->id,'deposite_user_id'=>$client->parent_id])->sum('amount');
        $depParent1 = DB::table('user_deposites')->where(['deposite_user_id'=>$client->id,'balanceType'=>'WITHDRAWAL'])->sum('amount');
        $depParent -= self::getProfitLossDL($userID,0);
        if($client->roles->first()->name == 'master'){
            $depParent += self::getProfitLossADMIN($userID,0);
        }else{
            $depParent -= self::getProfitLossDL($userID,0);
        }
        
//        $depParent = ($depParent1+$depParent)-$widTot;
...
//        dd($abc);
    ...
    public static function getChildAvalableBalanceADMINNew($userID){
      $model = User::where(['parent_id'=>$userID])->get();
      $avBTot = 0;
//      dd($model);
      foreach($model as $key=>$user){
        $depParent = self::getDepositeBalnceADMIN($user->id);
//        $exposureTot = self::getExAmountPerByUser($user->id);
    ...
    public static function getChildAvalableBalanceADMIN($userID){
      $model = User::where(['parent_id'=>$userID])->get();
      $avBTot = 0;
//      dd($model);
      foreach($model as $key=>$user){
    ...
    public static function getUserDownLevelBalance($userID){
      ...
      foreach($parentUser as $uKey=>$childUsers){
        $tot += self::getDepositeBalnceADMIN($childUsers->id);
      }
      return $tot;
    }
    ...
    public function listclients_OLD()
    {
...
//      return self::getExAmountPerByUser($user->parent_id,'UPPERLEVEL');
     ...
     * @return \Illuminate\Http\Response
     */
    public function addaccount_OLD()
    {
...
//        return $upl;
    ...
    public static function getProfitLossADMIN($userID,$isActive = 1){
        $upl = DB::table('upper_level_down_levels')->where(['user_id'=>$userID,'active'=>$isActive])->sum('downLevel');
//        return $upl;
        return ($upl*(-1));
    }
...
//                }
//        }else{
//            dd($model->parent_id."====".$childID);
            if($model->parent_id == $childID){
                $uplArr = \App\UpperLevelDownLevel::where(['user_id'=>$userID,'active'=>1])->get();
                if(is_object($uplArr) && count($uplArr) == 0){
                    $uplArr = \App\UpperLevelDownLevel::where(['user_id'=>$childID,'active'=>1])->get();
                    $isParent = true;
                }
            }else{
                $uplArr = \App\UpperLevelDownLevel::where(['user_id'=>$userID,'active'=>1])->get();
...
//      dd($depParent."==".$widTot."==".$pl."==".$pldeposite);
    ...
    /************** MAIN CLIENT DETAIL CAL ***************/
     ...
     public static function getMainCal($userId){
      ...
      /*** USER CREDIT LIMIT ****/
      $mainCalArr['CreditReferance'] = self::getNumberFormet($loginModel->credit_ref);
      
      /*** Down level Occupy Balance ****/
      
      $mainCalArr['DownLevelOccupyBalance'] = self::getNumberFormet(self::getChildAvalableBalanceADMINNew($userId));
      $mainCalArr['AvailableBalance'] = self::getNumberFormet($mainCalArr['MasterBalance']-$mainCalArr['DownLevelOccupyBalance']);
...
//        $mainCalArr['MasterBalance'] = $mainCalArr['MasterBalance'];
        ...
        $mainCalArr['UpperLevel'] = self::getNumberFormet($mainCalArr['MasterBalance']-$mainCalArr['CreditReferance']);
...
//          $plTot = ($plTot-$loginModel->myProfitLoss);
          $mainCalArr['AvailableBalanceWithProfitLoss'] = self::getNumberFormet($plTot);
        ...
        $mainCalArr['AvailableBalanceWithProfitLoss'] =  self::getNumberFormet($ABPLamt);
        $mainCalArr['MyProfitLoss'] = self::getMyProfit($userId);
      }
      $mainCalArr['MyProfitLoss'] = self::getNumberFormet($mainCalArr['MyProfitLoss']*(-1));
      $mainCalArr['DownLevelCreditReferance'] = self::getNumberFormet(self::getDownLevelCreditReferance($userId));
...
//      $mainCalArr['AvailableBalance'] = ($mainCalArr['MasterBalance']-$mainCalArr['DownLevelOccupyBalance']);
      ...
      $mainCalArr['DownLevelProfitLoss'] = self::getNumberFormet($DownLevelProfitLoss);
      ...
      if($loginModel->parent_id > 0){
        ...
        $mainCalArr['FixedLoss'] = self::getNumberFormet(($mainCalArr['CreditReferance']*($patnership))/100);
      }else{
        $mainCalArr['FixedLoss'] = 0;
      }
      $mainCalArr['FixedLoss'] = 0;
      $mainCalArr['MasterBalance'] = self::getNumberFormet($mainCalArr['MasterBalance']);
      return $mainCalArr;
    }
