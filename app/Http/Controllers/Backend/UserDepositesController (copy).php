...
//          $withdrawUserBaalanceAmount = $userData['AvailableBalance'];
          $withdrawUserBaalanceAmount = ListClientController::getUserAvalableBalanceDL($requestData['withdrawal_user_id']);
        }else{
...
//        dd($upperLevelAmt);
        ...
        if($upperLevelAmt !=0){
          if($upperLevelAmt > 0){
            ...
            $userModel = User::find($requestData['withdrawal_user_id']);
//            echo $userModel->roles->first()->name."<br>";
//            echo $upperLevelAmt."<br>";
            if($userModel->roles->first()->name == 'administrator'){
//                $upperLevelAmt = ListClientController::getProfitLossDL($requestData['deposite_user_id']);
...
//                dd($updownModel);
                ...
                $updownModel = UpperLevelDownLevel::where(['user_id'=>$requestData['deposite_user_id']])->get();
                }
            }
//            dd($updownModel);
            foreach($updownModel as $key=>$updown){
              if($updown->upperLevel <= $upperLevelAmt){
                $mod = UpperLevelDownLevel::find($updown->id);
                $mod->active = 0;
                $mod->save();
                $upperLevelAmt -=$updown->upperLevel;
              }else{
                if($upperLevelAmt > 0){
                  $mod = UpperLevelDownLevel::find($updown->id);
                  $mod->active = 0;
...
//                dd($upperLevelAmt);
                $childUserArr = AccountsController::getChildUserListArr($requestData['withdrawal_user_id']);
//                dd($childUserArr);
//                dd($updownModelDeposit);
//                $updownModel
                
                foreach($updownModelDeposit as $key=>$updown){
//                    echo "<br>".$updown->bet_user_id."<br>";
                    if(!in_array($updown->bet_user_id,$childUserArr)){
                        continue;
                    }
                    $userModelBetUser = User::find($updown->bet_user_id);
